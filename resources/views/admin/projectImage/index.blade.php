    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="main-title-00">
                ...
                @endif
                <h4 class="page-title">صور المشروع</h4>
            </div>
            ...
            <div class="card-box">
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class=" main-btn-00">
                            <!-- Responsive modal -->
                            <a href="{{ route('admin.projectImages.create', $id) }}"
                ...
                <div class="table-responsive">
                    <table data-toggle="table" data-search="true" data-show-columns="true" data-sort-name="id"
                        ...
                        <thead>
                            <tr>
                                <th data-field="الصورة الرئيسية" data-align="center"> صورة المشروع</th>
                        ...
                        <tbody>
                            ...
                            @foreach ($projectImages as $Image)
                                <tr>
                                    <td><img src="{{ asset('admin_assets/images/projects/' . $Image->image) }}"
                                            class="img-responsive" width="100px" height="100px"></td>
                                    <td>{{ $Image->status === 1 ? 'مفعل' : 'غير مفعل' }}</td>

                                    <td class="actions">
                                        {{-- <a href="{{ route('admin.changeStatus', [$Image->status, 'project_images', $Image->id]) }}"
                                            class="btn btn-{{ $Image->status == 1 ? 'secondary' : 'dark' }} waves-effect"
                                            title="الحالة"> {{ $Image->status == 1 ? 'إبطال' : 'تفعيل' }}</a> --}}

                                        <a href="{{ route('admin.changeStatus', [$Image->status, 'project_images', $Image->id]) }}"
                                            class="btn btn-{{ $Image->status == 1 ? 'secondary' : 'dark' }} waves-effect"
                                            title="الحالة"> {{ $Image->status == 1 ? 'إبطال' : 'تفعيل' }}</a>
                                        <a href="{{ route('admin.projectImages.edit', $Image->id) }}"
                                        ...
                                        <button type="button" class="btn btn-danger waves-effect" data-toggle="modal"
                                            data-target="#{{ $Image->id }}delete" title="حذف">حذف </button>
                                    </td>
                                </tr>

                                <div id="{{ $Image->id }}delete" class="modal fade" tabindex="-1" role="dialog"
                                    ...
                                    <div class="modal-dialog" style="width:55%;">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer" style="text-align:center">
                                                <form action="{{ route('admin.projectImages.destroy', $Image->id) }}"
                                                    method="post">
                                                    @csrf
