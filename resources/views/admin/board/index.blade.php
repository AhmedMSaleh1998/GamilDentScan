    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="main-title-00">
                ...
                @endif
                <h4 class="page-title">اعضاء مجلس الإدارة</h4>
            </div>
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class=" main-btn-00">
                            <!-- Responsive modal -->
                            <a href="{{ route('admin.boards.create') }}" class="btn btn-default waves-effect">اضافه عضو
                                +</a>
                        </div>
                    </div>
                </div>
                    ...
                    <table data-toggle="table" data-search="true" data-show-columns="true" data-sort-name="id"
                        ...
                        <thead>
                            <tr>
                                ...
                                <th data-field="اسم المشروع عربي" data-align="center">اسم العضو عربي</th>
                                <th data-field="اسم المشروع انجليزي" data-align="center">اسم العضو انجليزي</th>
                        ...
                        <tbody>
                            @if (isset($boards)) -
                                @foreach ($boards as $board)
                                    <tr>
                                        <td><img src="{{ asset('admin_assets/images/boards/' . $board->image) }}"
                                                class="img-responsive" width="100px" height="100px"></td>
                                        <td>{{ $board->name_ar }}</td>
                                        <td>{{ $board->name_en }}</td>
                                        <td>{{ $board->status === 1 ? 'مفعل' : 'غير مفعل' }}</td>

                                        <td class="actions">
                                            <a href="{{ route('admin.changeStatus', [$board->status, 'boards', $board->id]) }}"
                                                class="btn btn-{{ $board->status == 1 ? 'secondary' : 'dark' }} waves-effect"
                                                title="الحالة"> {{ $board->status == 1 ? 'إبطال' : 'تفعيل' }}</a>
                                            <a href="{{ route('admin.boards.show', $board->id) }}"
                                            ...
                                            <button type="button" class="btn btn-danger waves-effect" data-toggle="modal"
                                                data-target="#{{ $board->id }}delete" title="حذف">حذف </button>
                                        </td>
                                    </tr>

                                    <div id="{{ $board->id }}delete" class="modal fade" tabindex="-1" role="dialog"
                                        ...
                                        <div class="modal-dialog" style="width:55%;">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-footer" style="text-align:center">
                                                    <form action="{{ route('admin.boards.destroy', $board->id) }}"
                                                        method="post">
                                                        @csrf
